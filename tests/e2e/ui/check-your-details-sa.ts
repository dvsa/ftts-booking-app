...
import {
  generalTitle, generalTitleNI, Languages, setRequestTimeout,
} from '../data/constants';
import {
  verifyExactText, setCookie, verifyContainsText, link, verifyIsNotVisible, click, verifyIsVisible, capitalizeFirstLetter, verifyTitleContainsText, getFutureDate,
} from '../utils/helpers';
...
import {
  Language, Locale, PreferredDay, PreferredLocation, SupportType, Target, TestType, Voiceover,
...
test('Verify page UI contents are displayed correctly', async (t) => {
  ...
  await verifyExactText(checkYourDetailsPage.testDetailsKey, 'On-screen British Sign Language', 6);
  await verifyExactText(checkYourDetailsPage.testDetailsKey, 'Voiceover language', 7);
...
test('Change test type - cancel using the link in the banner, the original test type is kept', async (t) => {
  ...
  await verifyExactText(checkYourDetailsPage.testDetailsValue, capitalizeFirstLetter(sessionData.currentBooking.testType), 0);
});
...
test('Change test type - save and we have to re-enter all other details', async (t) => {
  ...
  await t.expect(Selector(languagePage.testLanguageButtonEnglish).checked).notOk();
  await t.expect(Selector(languagePage.testLanguageButtonWelsh).checked).notOk();
  await languagePage.selectTestLanguage(Languages.get(sessionData.currentBooking.language));

  const selectStandardSupportPage = new SelectStandardSupportPage();
  await t.expect(Selector(selectStandardSupportPage.onScreenBslRadioButton).checked).notOk();
  await t.expect(Selector(selectStandardSupportPage.voiceoverRadioButton).checked).notOk();
  await t.expect(Selector(selectStandardSupportPage.noSupportRadioButton).checked).notOk();
  await selectStandardSupportPage.selectSupportTypeRequired(SupportType.NO_SUPPORT_WANTED);
...
test('Change language for NI - change button is hidden for a NI user', async (t) => {
  ...
  await verifyTitleContainsText(`${checkYourDetailsPage.pageHeading} ${generalTitleNI}`);
  await verifyContainsText(checkYourDetailsPage.pageHeadingLocator, checkYourDetailsPage.pageHeading);
...
test('Change Voiceover NI - the correct content is displayed', async (t) => {
  ...
  const voiceoverPage = await checkYourDetailsPage.changeVoiceover();
  await verifyContainsText(voiceoverPage.pageHeadingLocator, voiceoverPage.pageHeading);
  await verifyIsNotVisible(voiceoverPage.backLink);
  const voiceoverSelector = voiceoverPage.voiceoverButton.replace('<>', sessionData.currentBooking.voiceover);
  await t.expect(Selector(voiceoverSelector).checked).ok();
...
test('Change Support Requested for GB from no to yes - save and the updated option is shown', async (t) => {
  const { sessionData } = t.ctx;
  const supportRequestedPage = await checkYourDetailsPage.changeSupportRequested();
