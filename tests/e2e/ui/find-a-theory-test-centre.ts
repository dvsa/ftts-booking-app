...
test('Verify page title and heading are displayed correctly', async () => {
  await verifyTitleContainsText(`${findATheoryTestCentrePage.pageTitle} ${Constants.generalTitle}`);
  await verifyExactText(findATheoryTestCentrePage.pageTitleLocator, findATheoryTestCentrePage.pageHeading);
});

test('Verify Back link takes you to the language page', async () => {
...
test('Verify the error message links to the search field for the user to navigate to', async () => {
  await clearField(findATheoryTestCentrePage.searchLocationTermTextBox);
  await click(findATheoryTestCentrePage.findButton);
  await verifyIsVisible(findATheoryTestCentrePage.errorLink);
...
test('Verify error prefix appears in the title when there is an error', async () => {
  await clearField(findATheoryTestCentrePage.searchLocationTermTextBox);
  await click(findATheoryTestCentrePage.findButton);
  await verifyTitleContainsText(`Error: ${findATheoryTestCentrePage.pageTitle} ${Constants.generalTitle}`);
});
