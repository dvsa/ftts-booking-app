...
import { InstructorVoicemailController } from '@pages/instructor-voicemail/instructor-voicemail';
...
describe('InstructorVoicemailController', () => {
  ...
  describe('get', () => {
    test('it should render the correct page', () => {
      ...
      expect(res.render).toHaveBeenCalledWith(PageNames.INSTRUCTOR_VOICEMAIL, expect.any(Object));
    });
    ...
    test('checks voicemail \'yes\' option if voicemail set to true in session', () => {
      ...
      expect(res.render).toHaveBeenCalledWith(PageNames.INSTRUCTOR_VOICEMAIL, expect.objectContaining({
        voicemailYes: true,
        voicemailNo: false,
      }));
    });
