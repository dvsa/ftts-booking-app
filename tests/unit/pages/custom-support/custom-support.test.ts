...
import { CustomSupportController } from '@pages/custom-support/custom-support';
...
describe('Custom support controller', () => {
  ...
  describe('GET request', () => {
    test('renders the custom support page with saved text', () => {
      ...
      expect(res.render).toHaveBeenCalledWith(PageNames.CUSTOM_SUPPORT, {
        errors: [],
    ...
    describe('skip link', () => {
      test('is set to confirm support if the user has selected other support only', () => {
        ...
        expect(res.render).toHaveBeenCalledWith(PageNames.CUSTOM_SUPPORT, expect.objectContaining({
          skipLink: 'confirm-support',
        }));
      });
