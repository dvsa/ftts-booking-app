...
describe('ChangeLocationTime', () => {
  ...
  describe('get', () => {
    test('renders the change location time template', () => {
      ...
      expect(res.render).toHaveBeenCalledWith(PageNames.CHANGE_LOCATION_TIME, {
        options: radioItems,
  ...
  describe('POST', () => {
    ...
    test('renders an error on the same template when validation fails', () => {
      ...
      expect(res.render).toHaveBeenCalledWith(PageNames.CHANGE_LOCATION_TIME, expect.objectContaining({
        errors: req.errors,
      }));
    });
