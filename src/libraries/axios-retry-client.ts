...
const is429Error = (err: AxiosError): boolean => err.response?.status === 429;
...
axiosRetry(axiosRetryClient, {
  ...
  retryDelay: (retryCount, err) => {
    ...
    logger.warn(`Retrying failed ${err.config?.method} request to ${err.config?.url} - attempt ${retryCount} of ${maxRetries}`, {
      error: err.toString(),
